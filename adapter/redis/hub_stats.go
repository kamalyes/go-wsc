 ...
 * @FilePath: \go-wsc\adapter\redis\hub_stats.go
...
// HubStats Redis Hub 统计仓储，实现 spi.HubStats 契约
...
// NewHubStats 创建 Redis Hub 统计仓库
// 参数:
//   - client: Redis 客户端 (github.com/redis/go-redis/v9)
//   - config: 统计配置对象
func NewHubStats(client redis.UniversalClient, config *wscconfig.Stats) *HubStats {
...
func (r *HubStats) UpdateConnectionStats(ctx context.Context, nodeID string, activeCount int64) error {
...
func (r *HubStats) IncrementTotalConnections(ctx context.Context, nodeID string, delta int64) error {
...
func (r *HubStats) SetActiveConnections(ctx context.Context, nodeID string, count int64) error {
...
func (r *HubStats) IncrementMessagesSent(ctx context.Context, nodeID string, delta int64) error {
	keys := []string{r.GetNodeKey(nodeID)}
	args := []any{
		FieldMessagesSent,
		delta,
...
func (r *HubStats) IncrementMessagesReceived(ctx context.Context, nodeID string, delta int64) error {
	keys := []string{r.GetNodeKey(nodeID)}
	args := []any{
...
func (r *HubStats) IncrementBroadcastsSent(ctx context.Context, nodeID string, delta int64) error {
	keys := []string{r.GetNodeKey(nodeID)}
	args := []any{
		FieldBroadcastsSent,
		delta,
...
func (r *HubStats) RegisterNode(ctx context.Context, nodeID string, startTime int64) error {
	keys := []string{r.GetNodeKey(nodeID), r.GetNodesSetKey()}
	args := []any{
		nodeID,
		startTime,
...
func (r *HubStats) UpdateNodeHeartbeat(ctx context.Context, nodeID string) error {
...
func (r *HubStats) GetNodeStats(ctx context.Context, nodeID string) (*models.NodeStats, error) {
	key := r.GetNodeKey(nodeID)

	// 使用Scan直接映射到结构体
	stats := &models.NodeStats{NodeID: nodeID}
...
func (r *HubStats) GetAllNodesStats(ctx context.Context) (map[string]*models.NodeStats, error) {
	// 获取所有节点ID
	...
	statsMap := make(map[string]*models.NodeStats)
...
func (r *HubStats) GetTotalStats(ctx context.Context) (*models.ClusterStats, error) {
	...
	clusterStats := &models.ClusterStats{
		...
		NodesStats: make([]*models.NodeStats, 0, len(allStats)),
		UpdateTime: time.Now(),
	}
...
func (r *HubStats) GetActiveNodes(ctx context.Context, timeout time.Duration) ([]string, error) {
...
var _ spi.HubStats = (*HubStats)(nil)
