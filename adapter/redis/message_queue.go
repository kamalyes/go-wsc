 ...
 * @FilePath: \go-wsc\adapter\redis\message_queue.go
...
// MessageQueue Redis 消息队列仓储，实现 spi.MessageQueue 契约
...
// NewMessageQueue 创建Redis消息队列仓库
func NewMessageQueue(client redis.UniversalClient, prefix string, ttl time.Duration) *MessageQueue {
...
// Enqueue 入队消息
func (r *MessageQueue) Enqueue(ctx context.Context, queueName string, msg *models.HubMessage) error {
	if msg == nil {
...
// Dequeue 出队消息(阻塞式)
func (r *MessageQueue) Dequeue(ctx context.Context, queueName string, timeout time.Duration) (*models.HubMessage, error) {
	key := r.prefix + queueName
...
func (r *MessageQueue) DequeueBatch(ctx context.Context, queueName string, count int) ([]*models.HubMessage, error) {
	if count <= 0 {
		return nil, nil
	}
	key := r.prefix + queueName
...
// GetLength 获取队列长度
func (r *MessageQueue) GetLength(ctx context.Context, queueName string) (int64, error) {
	key := r.prefix + queueName
...
// Clear 清空队列
func (r *MessageQueue) Clear(ctx context.Context, queueName string) error {
	key := r.prefix + queueName
...
// Peek 查看队列头部消息(不移除)
func (r *MessageQueue) Peek(ctx context.Context, queueName string) (*models.HubMessage, error) {
	key := r.prefix + queueName
