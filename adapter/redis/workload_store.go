 ...
 * @FilePath: \go-wsc\adapter\redis\workload_store.go
 * @Description: 客服负载管理 - Redis + DB 双层存储（支持多命名空间隔离）
 *
...
// WorkloadInfo 已迁至 models/contract.go —— 它是 spi.WorkloadStore 接口签名的
// 一部分，接口层不应反向依赖本包。

// WorkloadStore Redis + DB 双层负载存储，实现 spi.WorkloadStore 契约
// namespace 从 ctx 中提取（routing.NamespaceFromContext），空时兜底 DefaultNamespace
...
// NewWorkloadStore 创建 Redis 负载管理仓库
// 参数:
//   - client: Redis 客户端 (github.com/redis/go-redis/v9)
//   - config: 负载管理配置对象
//   - log: 日志记录器
func NewWorkloadStore(client redis.UniversalClient, db *gorm.DB, config *wscconfig.Workload, log logger.ILogger) *WorkloadStore {
...
func (r *WorkloadStore) namespaceFromCtx(ctx context.Context) string {
	return mathx.IfEmpty(routing.NamespaceFromContext(ctx), constants.DefaultNamespace)
}

// GetDimensionKey 生成指定维度的 Redis key
// 格式：{keyPrefix}{namespace}:{dimension}:{timeKey}:agent:{agentID}
// realtime: workload:{ns}:realtime:agent:{agentID}
// hourly: workload:{ns}:hourly:2026022513:agent:agent123
func (r *WorkloadStore) GetDimensionKey(namespace string, dimension models.WorkloadDimension, agentID string, t time.Time) string {
	return r.keyPrefix + namespace + ":" + dimension.FormatKeyPart(t) + ":agent:" + agentID
}

// GetDimensionZSetKey 生成指定维度的 ZSet key
// 格式：{keyPrefix}{namespace}:{dimension}:{timeKey}:zset
// realtime: workload:{ns}:realtime:zset
// hourly: workload:{ns}:hourly:2026022513:zset
func (r *WorkloadStore) GetDimensionZSetKey(namespace string, dimension models.WorkloadDimension, t time.Time) string {
	return r.keyPrefix + namespace + ":" + dimension.FormatKeyPart(t) + ":zset"
}

// evalLua 执行 Lua 脚本的通用方法
func (r *WorkloadStore) evalLua(ctx context.Context, script string, keys []string, args ...any) (any, error) {
	return r.client.Eval(ctx, script, keys, args...).Result()
}

// parseWorkloadResult 解析 Redis 返回的负载值
func (r *WorkloadStore) parseWorkloadResult(result any) int64 {
...
// 返回 realtime 维度的负载值
func (r *WorkloadStore) ReloadAgentWorkload(ctx context.Context, agentID string) (int64, error) {
	...
	if err == nil {
		...
		// 同步所有维度到 ZSet
		if err := r.syncAllDimensionsToZSet(ctx, namespace, agentID, existingWorkload, now); err != nil {
	...
	if r.db != nil {
		var dbModel models.AgentWorkloadModel
		...
		if dbErr == nil {
			...
			// 恢复所有维度到 Redis
			if err := r.setAllDimensions(ctx, namespace, agentID, finalWorkload, now); err != nil {
...
func (r *WorkloadStore) syncAllDimensionsToZSet(ctx context.Context, namespace, agentID string, workload int64, t time.Time) error {
	luaScript := `
		local keyPrefix = ARGV[1]
		local namespace = ARGV[2]
		local agentID = ARGV[3]
		local workload = tonumber(ARGV[4])

		-- 从 ARGV[5] 开始是维度配置：dimension, timeKey
		local argIndex = 5
		...
		while argIndex <= #ARGV do
			...
			local zsetKey
			if timeKey == "" then
				zsetKey = keyPrefix .. namespace .. ":" .. dimension .. ":zset"
			else
				zsetKey = keyPrefix .. namespace .. ":" .. dimension .. ":" .. timeKey .. ":zset"
			end
...
func (r *WorkloadStore) setAllDimensions(ctx context.Context, namespace, agentID string, workload int64, t time.Time) error {
	luaScript := `
		local keyPrefix = ARGV[1]
		local namespace = ARGV[2]
		local agentID = ARGV[3]
		...
		-- 从 ARGV[5] 开始是维度配置：dimension, timeKey, ttl(秒)
		local argIndex = 5
		...
		while argIndex <= #ARGV do
			...
			if timeKey == "" then
				workloadKey = keyPrefix .. namespace .. ":" .. dimension .. ":agent:" .. agentID
				zsetKey = keyPrefix .. namespace .. ":" .. dimension .. ":zset"
			else
				workloadKey = keyPrefix .. namespace .. ":" .. dimension .. ":" .. timeKey .. ":agent:" .. agentID
				zsetKey = keyPrefix .. namespace .. ":" .. dimension .. ":" .. timeKey .. ":zset"
			end
	...
	// 异步同步到 DB
	r.asyncSyncToDB(namespace, agentID, workload)
...
func (r *WorkloadStore) ForceSetAgentWorkload(ctx context.Context, agentID string, workload int64) error {
	namespace := r.namespaceFromCtx(ctx)
	now := time.Now()

	if err := r.setAllDimensions(ctx, namespace, agentID, workload, now); err != nil {
...
func (r *WorkloadStore) GetAgentWorkload(ctx context.Context, agentID string) (int64, error) {
	namespace := r.namespaceFromCtx(ctx)
	now := time.Now()
	realtimeKey := r.GetDimensionKey(namespace, models.WorkloadDimensionRealtime, agentID, now)
...
func (r *WorkloadStore) IncrementAgentWorkload(ctx context.Context, agentID string) error {
	namespace := r.namespaceFromCtx(ctx)
	return r.incrementMultiDimension(ctx, namespace, agentID, 1)
}

// incrementMultiDimension 多维度增加客服负载（原子操作）
func (r *WorkloadStore) incrementMultiDimension(ctx context.Context, namespace, agentID string, delta int64) error {
	now := time.Now()
	...
	luaScript := `
		...
		local dimensions = {
			{"realtime", "", 0},
			{"hourly", ARGV[6], 604800},      -- 7天
			{"daily", ARGV[7], 7776000},      -- 90天
			{"monthly", ARGV[8], 63072000},   -- 2年
			{"yearly", ARGV[9], 157680000}    -- 5年
		}
	...
	if resultArray, ok := result.([]any); ok {
		for _, item := range resultArray {
			if dimResult, ok := item.([]any); ok && len(dimResult) == 3 {
				...
				// 异步同步到 DB
				r.asyncSyncMultiDimensionToDB(namespace, agentID, models.WorkloadDimension(dimension), timeKey, workload)
			}
		}
	}
...
func (r *WorkloadStore) DecrementAgentWorkload(ctx context.Context, agentID string) error {
	namespace := r.namespaceFromCtx(ctx)
	return r.incrementMultiDimension(ctx, namespace, agentID, -1)
}

// GetLeastLoadedAgent 获取负载最小的在线客服（支持多策略）
func (r *WorkloadStore) GetLeastLoadedAgent(ctx context.Context, onlineAgents []string, dimension models.WorkloadDimension) (string, int64, error) {
	namespace := r.namespaceFromCtx(ctx)
	if len(onlineAgents) == 0 {
		return "", 0, errorx.WrapError("no online agents available")
	}
	return r.getLeastLoadedAgentFromRedis(ctx, namespace, onlineAgents, dimension, time.Now())
}

// getLeastLoadedAgentFromRedis 从 Redis ZSet 获取负载最小的客服（支持多维度）
func (r *WorkloadStore) getLeastLoadedAgentFromRedis(ctx context.Context, namespace string, onlineAgents []string, dimension models.WorkloadDimension, t time.Time) (string, int64, error) {
	...
	luaScript := `
		...
		local dimension = KEYS[4]
		local timeKey = KEYS[5]
		...
		local function buildWorkloadKey(agentID)
			if timeKey == "" then
				return keyPrefix .. namespace .. ":" .. dimension .. ":agent:" .. agentID
			else
				return keyPrefix .. namespace .. ":" .. dimension .. ":" .. timeKey .. ":agent:" .. agentID
			end
		end
	...
	// 获取指定维度的 ZSet key
	zsetKey := r.GetDimensionZSetKey(namespace, dimension, t)
	...
	// 执行 Lua 脚本
	result, err := r.evalLua(ctx, luaScript, []string{zsetKey, keyPrefix, namespace, dimension.String(), timeKey}, args...)
...
func (r *WorkloadStore) AcquireLeastLoadedAgent(ctx context.Context, onlineAgents []string, dimension models.WorkloadDimension) (string, int64, error) {
	namespace := r.namespaceFromCtx(ctx)
	if len(onlineAgents) == 0 {
		return "", 0, errorx.WrapError("no online agents available")
	}
	...
	luaScript := `
		local keyPrefix = ARGV[1]
		local namespace = ARGV[2]
		local realtimeTimeKey = ARGV[3]
		local hourlyTimeKey = ARGV[4]
		local dailyTimeKey = ARGV[5]
		local monthlyTimeKey = ARGV[6]
		local yearlyTimeKey = ARGV[7]
		-- ARGV[8..] 为在线客服 ID 列表
		...
		local function buildZSetKey(dimension, timeKey)
			if timeKey == "" then
				return keyPrefix .. namespace .. ":" .. dimension .. ":zset"
			else
				return keyPrefix .. namespace .. ":" .. dimension .. ":" .. timeKey .. ":zset"
			end
		end
	...
	args := make([]any, 0, 7+len(onlineAgents))
	args = append(args,
		r.keyPrefix,
		namespace,
...
//     用于统计的累计数据，不应在下线时清除
func (r *WorkloadStore) RemoveAgentWorkload(ctx context.Context, agentID string) error {
	namespace := r.namespaceFromCtx(ctx)
	now := time.Now()
...
func (r *WorkloadStore) GetAllAgentWorkloads(ctx context.Context, limit int64) ([]models.WorkloadInfo, error) {
	namespace := r.namespaceFromCtx(ctx)
	...
	workloads := make([]models.WorkloadInfo, 0, len(results))
	for _, z := range results {
		workloads = append(workloads, models.WorkloadInfo{
...
// Close 关闭仓库
func (r *WorkloadStore) Close() error {
	r.logger.Info("🛑 WorkloadStore 已关闭")
	return nil
}

// asyncSyncToDB 异步同步负载到 DB（多维度）
func (r *WorkloadStore) asyncSyncToDB(namespace, agentID string, workload int64) {
	now := time.Now()
...
func (r *WorkloadStore) asyncSyncMultiDimensionToDB(namespace, agentID string, dimension models.WorkloadDimension, timeKey string, workload int64) {
	if r.db == nil {
		return
	}
	...
	syncx.Go().WithTimeout(2 * time.Second).OnError(func(err error) {
		...
		// 唯一键: (namespace, agent_id, dimension, time_key)
		// 通过 Dialect 引擎兼容 MySQL/SQLite/PostgreSQL 的 UpsertColumnRef
		// - MySQL:  VALUES(workload)
		// - SQLite/PG:  excluded.workload
		dialect := sqlbuilder.DetectDialect(r.db)
		return r.db.WithContext(ctx).Clauses(clause.OnConflict{
			Columns: []clause.Column{{Name: "namespace"}, {Name: "agent_id"}, {Name: "dimension"}, {Name: "time_key"}},
			DoUpdates: clause.Assignments(map[string]any{
				"workload":   gorm.Expr(dialect.UpsertColumnRef("workload")),
				"updated_at": gorm.Expr(dialect.UpsertColumnRef("updated_at")),
			}),
		}).Create(&models.AgentWorkloadModel{
			Namespace:         namespace,
			AgentID:           agentID,
...
var _ spi.WorkloadStore = (*WorkloadStore)(nil)
