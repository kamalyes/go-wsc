 ...
 * @LastEditTime: 2026-09-17 23:00:26
 * @FilePath: \go-wsc\batcher\heartbeat_batcher.go
...
func NewHeartbeatStatsUpdater(hub StorageBatchWriter, queueSize, batchSize int, flushInterval time.Duration) *HeartbeatStatsUpdater {
...
func (u *HeartbeatStatsUpdater) flush(batch []*HeartbeatStatsEntry) {
	...
	// 转换为仓储层条目
	entries := make([]*models.HeartbeatUpdateEntry, 0, len(deduped))
	for _, entry := range deduped {
		// 复制局部变量避免循环变量逃逸问题
		pingTime := entry.PingTime
		pongTime := entry.PongTime
		entries = append(entries, &models.HeartbeatUpdateEntry{
	...
	if u.hub.GetConnectionRecordRepo() != nil {
		if err := u.hub.GetConnectionRecordRepo().BatchUpdateHeartbeats(ctx, entries); err != nil {
			u.hub.GetLogger().ErrorKV("批量更新心跳时间戳失败",
				"error", err,
	...
	if u.hub.GetConnectionQualityRepository() != nil {
		if err := u.hub.GetConnectionQualityRepository().BatchUpdateHeartbeats(ctx, entries); err != nil {
			u.hub.GetLogger().ErrorKV("批量更新心跳统计失败",
				"error", err,
				"batch_size", len(entries),
			)
		}
