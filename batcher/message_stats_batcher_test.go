 ...
 * @LastEditTime: 2026-09-19 00:36:17
 * @FilePath: \go-wsc\batcher\message_stats_batcher_test.go
...
// newStatsTestHub 创建带假仓储的端口替身（本包不依赖真实 Hub）
func newStatsTestHub(t *testing.T) (*fakeBatchWriter, *fakeQualityStore) {
	t.Helper()
...
func TestNewMessageStatsBatcher(t *testing.T) {
	...
	batcher := NewMessageStatsBatcher(w, 64, 8, 50*time.Millisecond)

	require.NotNil(t, batcher, "创建的 batcher 不应为 nil")
	assert.Same(t, w, batcher.hub, "batcher.hub 应指向传入的端口")
...
func TestMessageStatsBatcher_SubmitSuccess(t *testing.T) {
	...
	batcher := NewMessageStatsBatcher(w, 100, 100, 10*time.Second)
...
func TestMessageStatsBatcher_FlushEmptyBatch(t *testing.T) {
	...
	batcher := NewMessageStatsBatcher(w, 10, 10, 10*time.Second)
	...
	assert.Empty(t, fakeRepo.incrementsSnapshot(), "空 batch 不应调用 BatchIncrementStats")
}
...
func TestMessageStatsBatcher_FlushAggregatesByConnectionID(t *testing.T) {
	...
	entries := fakeRepo.incrementsSnapshot()
	require.Len(t, entries, 3, "flush 后应聚合为 3 个 connectionID")

	agg := make(map[string]*models.StatsIncrementEntry, len(entries))
	for _, e := range entries {
		agg[e.ConnectionID] = e
	}

	require.Contains(t, agg, "conn-a")
	require.Contains(t, agg, "conn-b")
	require.Contains(t, agg, "conn-c")
...
func TestMessageStatsBatcher_SubmitAndAutoFlush(t *testing.T) {
	...
	batcher := NewMessageStatsBatcher(w, 100, 2, 50*time.Millisecond)
	...
	entries := fakeRepo.incrementsSnapshot()
	agg := make(map[string]*models.StatsIncrementEntry)
	for _, e := range entries {
		existing, ok := agg[e.ConnectionID]
		if !ok {
			cp := *e
			agg[e.ConnectionID] = &cp
		} else {
	...
	require.Contains(t, agg, "conn-auto-1")
	assert.Equal(t, int64(2), agg["conn-auto-1"].MessagesSent, "conn-auto-1 跨多次 flush 汇总应为 1+1=2")
	assert.Equal(t, int64(400), agg["conn-auto-1"].BytesSent, "conn-auto-1 BytesSent 跨多次 flush 汇总应为 100+300=400")
	require.Contains(t, agg, "conn-auto-2")
	assert.Equal(t, int64(1), agg["conn-auto-2"].MessagesSent)
	assert.Equal(t, int64(200), agg["conn-auto-2"].BytesSent)
}
...
func TestMessageStatsBatcher_StopFlushesRemaining(t *testing.T) {
	...
	batcher := NewMessageStatsBatcher(w, 1000, 1000, 10*time.Second)
