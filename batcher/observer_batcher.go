 ...
 * @LastEditTime: 2026-09-19 22:00:00
 * @FilePath: \go-wsc\batcher\observer_batcher.go
...
type ObserverNotificationBatcher struct {
	hub       ObserverNotifier
	processor *syncx.BatchProcessor[*ObserverNotifyItem]
}

// NewObserverNotificationBatcher 创建观察者通知批量处理器
func NewObserverNotificationBatcher(hub ObserverNotifier, queueSize, batchSize int, flushInterval time.Duration) *ObserverNotificationBatcher {
...
// Hub 返回本批处理器绑定的通知端口（观测/测试断言用）
func (b *ObserverNotificationBatcher) Hub() ObserverNotifier { return b.hub }

// Processor 返回底层批处理器（观测/测试断言用）
func (b *ObserverNotificationBatcher) Processor() *syncx.BatchProcessor[*ObserverNotifyItem] {
	return b.processor
}
...
func (b *ObserverNotificationBatcher) Submit(msg *models.HubMessage, namespace string, groupIDs []string) bool {
	if b == nil || msg == nil {
		return false
	}
