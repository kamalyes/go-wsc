...
// grpcKey/heartbeatKey 来自 go-config Cluster 配置，默认 "wsc:nodes:grpc" / "wsc:nodes:heartbeat"
func NewNodeRegistry(redisClient redis.UniversalClient, nodeID, grpcAddr, grpcKey, heartbeatKey string, logger spi.Logger) *NodeRegistry {
	return &NodeRegistry{
...
// 配置端口为 0（随机端口）时，监听器绑定后才能得知实际端口，
// 必须在 Register 之前回填，否则注册进发现表的是无效地址
func (r *NodeRegistry) SetGRPCAddr(addr string) {
	r.grpcAddr = addr
}

// SetNodeAddr 手动登记某节点的 gRPC 地址
//
// 与 Redis 自动发现互补：适用于静态节点列表部署，或在测试中绕过
// Redis 发现刷新延迟直接建立节点映射
func (r *NodeRegistry) SetNodeAddr(nodeID, addr string) {
