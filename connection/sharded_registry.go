 ...
 * @FilePath: \go-wsc\connection\sharded_registry.go
...
type ShardedRegistry struct {
	...
	userShards *syncx.ShardedMap[string, map[string]*models.Client]
	...
	sseShards *syncx.ShardedMap[string, map[string]*models.Client]

	// observerShards 观察者分类索引：userID → (clientID → Client)
	// 替代 Hub.observerClients，nil 表示该模块未启用（读写均跳过）
	observerShards *syncx.ShardedMap[string, map[string]*models.Client]
	...
	agentShards *syncx.ShardedMap[string, map[string]*models.Client]
...
func newClientShardedMap(totalHint int) *syncx.ShardedMap[string, map[string]*models.Client] {
	return syncx.NewShardedMapWithOptions(
		defaultShardCount,
		syncx.WithPerShardHint[string, map[string]*models.Client](RegistryCapacity{}.perShardHint(totalHint)),
	)
}
...
func (r *ShardedRegistry) AddClient(client *models.Client) {
	if client == nil {
		return
	}
	...
	isNew := false
	r.userShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
		userClients, exists := data[client.UserID]
		if !exists {
			userClients = make(map[string]*models.Client)
...
// 返回被移除的客户端（nil 表示不存在）
func (r *ShardedRegistry) RemoveClient(clientID, userID string) *models.Client {
	var removed *models.Client

	// 1. 主存储移除
	r.userShards.WithShardLock(userID, func(data map[string]map[string]*models.Client) {
...
func (r *ShardedRegistry) GetClient(clientID string) (*models.Client, bool) {
...
func (r *ShardedRegistry) GetUserClients(userID string) (map[string]*models.Client, bool) {
...
func (r *ShardedRegistry) ForEachUserClient(userID string, fn func(clientID string, client *models.Client) bool) {
	r.userShards.WithShardRLock(userID, func(data map[string]map[string]*models.Client) {
		userClients, ok := data[userID]
		if !ok {
			return
...
func (r *ShardedRegistry) MoveClientGroup(client *models.Client, newGroupID string) {
	if client.UserType == models.UserTypeObserver {
...
func (r *ShardedRegistry) GetAllClients() []*models.Client {
	result := make([]*models.Client, 0, r.clientCount.Load())

	r.userShards.Range(func(_ string, userClients map[string]*models.Client) bool {
...
func (r *ShardedRegistry) ForEachClient(fn func(clientID string, client *models.Client) bool) {
	r.userShards.Range(func(_ string, userClients map[string]*models.Client) bool {
		for clientID, client := range userClients {
			if !fn(clientID, client) {
				return false
...
// 回调返回 false 时停止遍历
// 注意：回调在持有读锁时执行，不应执行耗时操作
func (r *ShardedRegistry) ForEachUser(fn func(userID string, clients map[string]*models.Client) bool) {
	r.userShards.Range(func(userID string, userClients map[string]*models.Client) bool {
...
func (r *ShardedRegistry) GetSSEUserClients(userID string) (map[string]*models.Client, bool) {
	var clients map[string]*models.Client
	var exists bool

	r.sseShards.WithShardRLock(userID, func(data map[string]map[string]*models.Client) {
		clients, exists = data[userID]
	})
...
// 回调返回 false 时停止遍历
func (r *ShardedRegistry) ForEachSSEClient(fn func(userID string, clientID string, client *models.Client) bool) {
	r.sseShards.Range(func(userID string, userClients map[string]*models.Client) bool {
		for clientID, client := range userClients {
...
func (r *ShardedRegistry) ForEachSSEUserClient(userID string, fn func(clientID string, client *models.Client) bool) {
	r.sseShards.WithShardRLock(userID, func(data map[string]map[string]*models.Client) {
		userClients, ok := data[userID]
		if !ok {
			return
...
func (r *ShardedRegistry) addSSEClient(client *models.Client) {
	r.sseShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
		userClients, exists := data[client.UserID]
		if !exists {
			userClients = make(map[string]*models.Client)
			data[client.UserID] = userClients
...
func (r *ShardedRegistry) removeSSEClient(client *models.Client) {
	r.sseShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
		userClients, exists := data[client.UserID]
		if !exists {
			return
...
func (r *ShardedRegistry) GetObserverUserClients(userID string) (map[string]*models.Client, bool) {
	...
	r.observerShards.WithShardRLock(userID, func(data map[string]map[string]*models.Client) {
		clients, exists = data[userID]
	})
...
func (r *ShardedRegistry) ForEachObserver(fn func(userID string, clientID string, client *models.Client) bool) {
	if r.observerShards == nil {
		return
	}
	r.observerShards.Range(func(userID string, userClients map[string]*models.Client) bool {
		for clientID, client := range userClients {
			if !fn(userID, clientID, client) {
				return false
			}
...
func (r *ShardedRegistry) GetObserverDeviceCount() int {
	...
	count := 0
	r.observerShards.Range(func(_ string, userClients map[string]*models.Client) bool {
...
func (r *ShardedRegistry) addObserverClient(client *models.Client) {
	if r.observerShards == nil {
		return
	}
	r.observerShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
		userClients, exists := data[client.UserID]
		if !exists {
			userClients = make(map[string]*models.Client)
			data[client.UserID] = userClients
...
func (r *ShardedRegistry) removeObserverClient(client *models.Client) {
	if r.observerShards == nil {
		return
	}
	r.observerShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
...
func (r *ShardedRegistry) ForEachClientParallel(workerNum int, fn func(clientID string, client *models.Client)) {
	if fn == nil {
		return
	}
	...
	r.userShards.RangeParallel(workerNum, func(_ string, userClients map[string]*models.Client) {
		for clientID, client := range userClients {
			fn(clientID, client)
		}
	})
...
// workerNum <= 0 时使用 GOMAXPROCS；用户数低于 parallelThreshold 时退化为串行
func (r *ShardedRegistry) ForEachSSEClientParallel(workerNum int, fn func(userID, clientID string, client *models.Client)) {
	if fn == nil {
		return
	}
	if r.sseShards.Len() <= parallelThreshold {
		r.ForEachSSEClient(func(userID, clientID string, client *models.Client) bool {
			fn(userID, clientID, client)
			return true
		})
		return
	}
	r.sseShards.RangeParallel(workerNum, func(userID string, userClients map[string]*models.Client) {
		for clientID, client := range userClients {
			fn(userID, clientID, client)
		}
	})
...
// 基于 userShards.RangeParallel 直接在 shard 级别并行遍历，零额外分配
// workerNum <= 0 时使用 GOMAXPROCS；用户数低于 parallelThreshold 时退化为串行
func (r *ShardedRegistry) ForEachClientFilteredParallel(workerNum int, msgAppID, msgNamespace string, msgGroupIDs []string, fn func(clientID string, client *models.Client)) {
	if fn == nil {
		return
	}
	if r.userShards.Len() <= parallelThreshold {
		r.ForEachClientFiltered(msgAppID, msgNamespace, msgGroupIDs, func(clientID string, client *models.Client) bool {
			fn(clientID, client)
			return true
		})
		return
...
func (r *ShardedRegistry) GetObserversForMessage(namespace string, groupIDs ...string) []*models.Client {
	if r.observerShards == nil {
		return nil
	}
...
type observerIndex struct {
	mu          sync.RWMutex
	global      map[string]*models.Client            // clientID → Client（Namespace=="" 全局观察者）
	byNamespace map[string]map[string]*models.Client // namespace → (clientID → Client)
	byGroup     map[string]map[string]*models.Client // "namespace:groupID" → (clientID → Client)
}

// newObserverIndex 创建观察者三级索引
func newObserverIndex() observerIndex {
	return observerIndex{
		global:      make(map[string]*models.Client),
		byNamespace: make(map[string]map[string]*models.Client),
		byGroup:     make(map[string]map[string]*models.Client),
	}
}
...
// add 添加观察者到对应级别的索引
func (idx *observerIndex) add(client *models.Client) {
	idx.mu.Lock()
	defer idx.mu.Unlock()
	...
	if client.Namespace == "" {
		...
		if !ok {
			group = make(map[string]*models.Client)
		...
		if !ok {
			ns = make(map[string]*models.Client)
...
// remove 从索引中移除观察者
func (idx *observerIndex) remove(client *models.Client) {
	idx.mu.Lock()
	defer idx.mu.Unlock()
...
func (idx *observerIndex) getForMessage(namespace string, groupIDs ...string) []*models.Client {
	idx.mu.RLock()
	...
	result := make([]*models.Client, 0, estimate)
...
func (r *ShardedRegistry) GetAgentUserClients(userID string) (map[string]*models.Client, bool) {
	...
	r.agentShards.WithShardRLock(userID, func(data map[string]map[string]*models.Client) {
		clients, exists = data[userID]
	})
...
func (r *ShardedRegistry) addAgentClient(client *models.Client) {
	if r.agentShards == nil {
		return
	}
	r.agentShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
		userClients, exists := data[client.UserID]
		if !exists {
			userClients = make(map[string]*models.Client)
			data[client.UserID] = userClients
...
func (r *ShardedRegistry) removeAgentClient(client *models.Client) {
	if r.agentShards == nil {
		return
	}
	r.agentShards.WithShardLock(client.UserID, func(data map[string]map[string]*models.Client) {
...
func (r *ShardedRegistry) Clear() {
	...
	if r.observerShards != nil {
		...
		r.observerIdx.global = make(map[string]*models.Client)
		r.observerIdx.byNamespace = make(map[string]map[string]*models.Client)
		r.observerIdx.byGroup = make(map[string]map[string]*models.Client)
