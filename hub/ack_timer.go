...
func (h *Hub) cancelAckTimeout(key models.MessageRecordKey) {
	if h.ackTimeoutTimer == nil || key.MessageID == "" {
		return
	}
	h.ackTimeoutTimer.CancelByKey(ackTimerKey(key))
}

// makeAckTimeoutCallback 创建 ACK 超时回调
// 超时触发时：原子认领（ClaimStaleSending 状态守卫，多节点去重）→ 标记 AckTimeout → 转存离线
func (h *Hub) makeAckTimeoutCallback(key models.MessageRecordKey) func() {
	return func() {
		// ⏰ 终态清理 user_not_found 重路由守卫条目（见 self_heal.go，守卫是消息维度）
		h.rerouteGuard.Delete(key.MessageID)
		...
		claimed, err := h.messageRecordRepo.ClaimStaleSending(ctx, []models.MessageRecordKey{key},
		...
		record, rErr := h.messageRecordRepo.FindByMessageID(ctx, key)
