...
func setupStressHub(t *testing.T, withGroup bool) (*Hub, repository.GroupRepository, []*Client, func()) {
	...
	if withGroup {
		...
		require.NoError(t, hub.AddGroupMembers(routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace(stressNamespace).WithGroupIDs([]string{stressGroupID}).Inject(ctx), members))
	}
...
func makeStressClient(clientID, userID, namespace string) *Client {
	c := &Client{
		...
		LastSeen:       time.Now(),
		AppID:          constants.DefaultAppID, // 与入口层归一化策略一致（msg 经 InjectRoute 归一化为 DefaultAppID）
	}
...
func TestConcurrentBroadcastStress(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < eachCount; i++ {
			...
			msg.SetBroadcastType(models.BroadcastTypeGlobal)
			nsCtx := routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace(stressNamespace).WithGroupIDs(nil).Inject(ctx)
	...
	go func() {
		...
		for i := 0; i < eachCount; i++ {
			...
			msg.RequireAck = true
			gctx := routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace(stressNamespace).WithGroupIDs([]string{stressGroupID}).Inject(ctx)
