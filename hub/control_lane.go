 ...
 * @FilePath: \go-wsc\hub\control_lane.go
...
func (h *Hub) SendControl(client *Client, data []byte, msg *models.HubMessage) bool {
	if client == nil {
		return false
	}
	...
	if msg != nil && IsDisconnectMessage(msg.MessageType) && client.Conn != nil {
		h.logger.WarnContextKV(client.Context, "控制通道满，断链类消息降级为直接断链",
			"client_id", client.ID,
...
func (h *Hub) SendControlMessage(client *Client, msg *models.HubMessage) bool {
	...
	if err != nil {
		h.logger.ErrorContextKV(client.Context, "控制消息序列化失败",
			"client_id", client.ID,
			"message_type", msg.MessageType,
			"error", err,
		)
	...
	return h.SendControl(client, data, msg)
}
