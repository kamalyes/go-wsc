 ...
 * @FilePath: \go-wsc\hub\delay_queue.go
...
type broadcastDelayEntry struct {
	ctx   context.Context
	msg   *HubMessage
	retry func(ctx context.Context, msg *HubMessage) // 重投函数（只做投递，不重新 Admit）
}
...
// newBroadcastDelayQueue 创建延迟队列（drain 循环由 Hub Run 启动）
func newBroadcastDelayQueue(h *Hub) *broadcastDelayQueue {
...
func (q *broadcastDelayQueue) offer(ctx context.Context, msg *HubMessage, retry func(ctx context.Context, msg *HubMessage)) bool {
	select {
...
func (q *broadcastDelayQueue) drainLoop(ctx context.Context) {
	for {
		...
		case entry := <-q.entries:
			...
			if shaper := q.hub.broadcastShaper.Load(); shaper != nil {
...
func (h *Hub) deferBroadcast(ctx context.Context, msg *HubMessage, retry func(context.Context, *HubMessage)) {
	...
	h.logger.ErrorContextKV(ctx, "广播延迟队列满：极端过载，消息降级",
		"message_id", msg.MessageID,
		"guarantee", guarantee,
		"queue_backlog", h.broadcastDelayQueue.Backlog(),
	)
}
