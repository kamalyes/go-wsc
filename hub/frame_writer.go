 ...
 * @LastEditTime: 2026-09-11 22:30:00
 * @FilePath: \go-wsc\hub\frame_writer.go
...
func (h *Hub) writeFramesBatch(conn *websocket.Conn, msgs [][]byte) error {
	if len(msgs) == 0 {
		return nil
	}
...
//
// 首条 WriteMessage 直写：无积压时与原路径完全一致（1 次syscall 低延迟）；
// 排空积压时收集 up to clientWriteBatchSize-1 条走 writev 合帧。
// 性能：突发 N 条 → 2 次 syscall（原 N 次）；GC 压力：帧头池化零分配
func (h *Hub) writeClientMessagesBatch(client *Client, first []byte) error {
	...
	for i := 1; i < clientWriteBatchSize; i++ {
		...
		default:
			...
			if err := h.writeFramesBatch(client.Conn, frames); err != nil {
				h.releaseBatchFrames(frames)
				return err
			}
			h.releaseBatchFrames(frames)
	...
	client.SetBacklogRatio(len(client.SendChan), cap(client.SendChan))
	h.onWriteBatch(batch)
	err := h.writeFramesBatch(client.Conn, frames)
	h.releaseBatchFrames(frames)
	return err
}
