...
func TestGRPCServer_BroadcastGroup_MemberFiltering(t *testing.T) {
	...
	require.NoError(t, hub.AddGroupMembers(routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace("ns-srv").WithGroupIDs([]string{"g-srv"}).Inject(ctx), []string{"u-m1", "u-m2"}))
...
func TestGRPCServer_BroadcastGroup_ExcludeSender(t *testing.T) {
	...
	require.NoError(t, hub.AddGroupMembers(routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace("ns-exc").WithGroupIDs([]string{"g-exc"}).Inject(ctx), []string{"u-sender", "u-other"}))
