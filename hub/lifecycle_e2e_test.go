...
func TestE2E_Connect_SendOnline_Receive(t *testing.T) {
	...
	assert.Equal(t, constants.DefaultAppID, clientB.AppID, "老系统 AppID 应归一化")
	assert.Equal(t, constants.DefaultNamespace, clientB.Namespace, "老系统 Namespace 应归一化")
...
func TestE2E_Legacy_NoDimensions_RealConn_P2P(t *testing.T) {
	...
	// 验证注册后字段归一化（真实连接路径也走 handleRegister 归一化）
	assert.Equal(t, constants.DefaultAppID, clientK.AppID, "老系统 AppID 应归一化")
	assert.Equal(t, constants.DefaultNamespace, clientK.Namespace, "老系统 Namespace 应归一化")
