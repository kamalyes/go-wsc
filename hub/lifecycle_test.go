...
func TestProcessPendingMessages_NormalForward(t *testing.T) {
	t.Parallel()
	hub := newMinHub()
	defer hub.Shutdown()

	// 启动 processPendingMessages（阻塞，需 goroutine）
	go hub.processPendingMessages()

	// 发送消息到 pendingMessages → 应转发到 broadcast
	msg := makeGroupMessage("sender-pending")
	hub.pendingMessages <- msg

	// 验证消息出现在 broadcast 队列
	require.Eventually(t, func() bool {
		select {
		case bcastMsg := <-hub.broadcast:
			return bcastMsg == msg
		default:
			return false
		}
...
func TestProcessPendingMessages_TimeoutPath(t *testing.T) {
	t.Parallel()
	config := wscconfig.Default().
		WithNodeInfo("127.0.0.1", 18080).
		WithHeartbeatInterval(30 * time.Second).
		WithMessageBufferSize(1) // broadcast 容量=1，容易填满
	hub := NewHub(config)
	defer hub.Shutdown()

	// 启动 processPendingMessages
	go hub.processPendingMessages()

	// 填满 broadcast 队列（容量=1）
	hub.broadcast <- makeGroupMessage("fill")

	// 发送到 pendingMessages → broadcast 满 → 进入超时等待路径
	// 5 秒超时后会记录 timeout 日志但不 panic
	pendingMsg := makeGroupMessage("timeout-test")
	hub.pendingMessages <- pendingMsg

	// 等待超时处理完成（5s 超时 + 余量）
	// 超时后消息会被丢弃，broadcast 中仍是 fill 消息
	time.Sleep(5500 * time.Millisecond)

	// broadcast 队列中应该是 fill 消息（timeout-test 被丢弃）
	select {
	case bcastMsg := <-hub.broadcast:
		assert.Equal(t, "fill", bcastMsg.Sender, "broadcast 中应是 fill 消息")
	default:
		t.Fatal("broadcast 队列不应为空")
	}
}
