...
func TestLegacySystem_RegisterNormalizesDimensions(t *testing.T) {
	...
	// 1. 验证 client 字段被归一化
	assert.Equal(t, models.DefaultAppID, client.AppID, "AppID 空应归一化为 DefaultAppID")
	assert.Equal(t, models.DefaultNamespace, client.Namespace, "Namespace 空应归一化为 DefaultNamespace")

	// 2. 验证 client 存储可查
	retrieved, exists := hub.shardedRegistry.GetClient(client.ID)
	require.True(t, exists, "注册后应能从 registry 查到 client")
	assert.Equal(t, models.DefaultAppID, retrieved.AppID, "registry 里的 AppID 应为归一化后的值")
	assert.Equal(t, models.DefaultNamespace, retrieved.Namespace, "registry 里的 Namespace 应为归一化后的值")
}
...
func TestLegacySystem_P2PSendWithBackgroundCtx(t *testing.T) {
	...
	assert.Equal(t, models.DefaultNamespace, ns, "老系统不传 namespace 应兜底为 DefaultNamespace")
...
func TestLegacySystem_EmptyGroupID_NoBogusGroupEntry(t *testing.T) {
	...
	assert.Equal(t, models.DefaultAppID, routing.AppIDFromContext(ctx2), "全空 ctx EnsureRouteDefaults 应补 DefaultAppID")
	assert.Equal(t, models.DefaultNamespace, routing.NamespaceFromContext(ctx2), "全空 ctx EnsureRouteDefaults 应补 DefaultNamespace")
...
func TestLegacySystem_OnlineDeliveryWithBackgroundCtx(t *testing.T) {
	...
	require.Equal(t, models.DefaultAppID, client.AppID)
	require.Equal(t, models.DefaultNamespace, client.Namespace)
