...
// 本地检查走 shardedRegistry.HasUser，跨节点检查走 onlineStatusRepo（同样按信封过滤）
...
func (h *Hub) checkUserOnline(ctx context.Context, userID string) bool {
	...
	if h.onlineStatusRepo != nil {
		...
		nodes, err := h.onlineStatusRepo.GetUserNodes(rctx, userID)
		if err == nil && len(nodes) > 0 {
			// 用户在其他节点在线（GetUserNodes 已按 appID+namespace 信封过滤）
			return true
		}
	}
