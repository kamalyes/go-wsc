...
func TestSendToGroup_InjectGroupForOfflineDimension(t *testing.T) {
	...
	require.NoError(t, hub.AddGroupMembers(routing.NewRoute().WithAppID(models.DefaultAppID).WithNamespace("tenantA").WithGroupIDs([]string{"g-dim"}).Inject(ctx), []string{"u-offline-dim"}))
	...
	msg.RequireAck = true
	groupCtx := routing.NewRoute().WithAppID(models.DefaultAppID).WithNamespace("tenantA").WithGroupIDs([]string{"g-dim"}).Inject(ctx)
