...
// 辅助函数：创建测试用的观察者客户端
func createObserverClient(id, userID string) *Client {
	return &Client{
		ID:             id,
		UserID:         userID,
...
// 辅助函数：批量创建观察者
func createObservers(count int, userPrefix string) []*Client {
	observers := make([]*Client, count)
	for i := 0; i < count; i++ {
		observers[i] = createObserverClient(
			fmt.Sprintf("observer-%d", i),
			fmt.Sprintf("%s-%d", userPrefix, i),
		)
	}
	return observers
}
...
func TestObserverBasicFunctionality(t *testing.T) {
	...
	observer := createObserverClient("observer-001", "observer-user-1")
	hub.Register(observer)
	time.Sleep(50 * time.Millisecond)

	assert.True(t, hub.IsObserver("observer-user-1"))
	...
	assert.False(t, hub.IsObserver("observer-user-1"))
...
func TestObserverMultiDevice(t *testing.T) {
	...
	// 同一观察者从3个设备登录
	observers := []*Client{
		createObserverClient("observer-001", "observer-user-1"),
		createObserverClient("observer-002", "observer-user-1"),
		createObserverClient("observer-003", "observer-user-1"),
	}
...
func TestObserverStats(t *testing.T) {
	...
	// 创建2个观察者用户，5个设备
	clients := []*Client{
		createObserverClient("obs-1-dev-1", "observer-1"),
		createObserverClient("obs-1-dev-2", "observer-1"),
		createObserverClient("obs-2-dev-1", "observer-2"),
		createObserverClient("obs-2-dev-2", "observer-2"),
		createObserverClient("obs-2-dev-3", "observer-2"),
	}
...
func TestObserverWithNormalUsers(t *testing.T) {
	...
	clients := []*Client{
		createObserverClient("observer-001", "observer-user-1"),
		createTestClient("customer-001", "customer-1", UserTypeCustomer),
		createTestClient("agent-001", "agent-1", UserTypeAgent),
	}
	registerClients(hub, clients)
	time.Sleep(50 * time.Millisecond)
...
func TestObserverReceiveMessages(t *testing.T) {
	...
	msg := NewHubMessage().
		...
		SetSender("user-1").
		SetReceiver("user-2").
		SetContent("Hello from user1")

	ctx := context.Background()
	result := hub.SendToUserWithRetry(ctx, "user-2", msg)
...
func TestObserverReceiveDistributedMessages(t *testing.T) {
	...
	observer1 := createObserverClient("observer-001", "observer-user-1")
	user2 := createTestClient("user-002", "user-2", UserTypeCustomer)
	hub1.Register(observer1)
	hub1.Register(user2)

	// 节点2: 注册 user-1（发送者）
	user1 := createTestClient("user-001", "user-1", UserTypeCustomer)
	hub2.Register(user1)
	...
	msg := NewHubMessage().
		SetID("msg-distributed-001").
		SetSender("user-1").
		SetReceiver("user-2").
		SetContent("Hello from node-2")

	result := hub2.SendToUserWithRetry(ctx, "user-2", msg)
	assert.NoError(t, result.FinalError)
	...
	// 验证节点1的观察者收到了消息
	assert.Greater(t, len(observer1.SendChan), 0, "节点1的观察者应该收到节点2的消息")
	...
	case msgData := <-observer1.SendChan:
		...
		// 验证消息内容
		assert.Equal(t, "Hello from node-2", receivedMsg.Content)
		assert.Equal(t, "user-1", receivedMsg.Sender)
		assert.Equal(t, "user-2", receivedMsg.Receiver)
...
func TestObserverMultiNodeReceive(t *testing.T) {
	...
	// 节点2: 注册观察者2
	observer2 := createObserverClient("observer-002", "observer-user-2")
	...
	msg := NewHubMessage().
		SetID("msg-multi-node-001").
		SetSender("user-1").
		SetReceiver("user-2").
		SetContent("Multi-node test message")

	result := hub3.SendToUserWithRetry(ctx, "user-2", msg)
	assert.NoError(t, result.FinalError)
...
func TestObserverReceiveBroadcastMessages(t *testing.T) {
	...
	case msgData := <-observer1.SendChan:
		...
		// 验证消息内容
		assert.Equal(t, "Global broadcast message", receivedMsg.Content)
		assert.Equal(t, "system", receivedMsg.Sender)
		assert.Equal(t, BroadcastTypeGlobal, receivedMsg.BroadcastType)
	default:
		t.Fatal("观察者没有收到广播消息")
	}
}
