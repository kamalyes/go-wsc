...
func TestBroadcastToGroup(t *testing.T) {
	...
	require.NoError(t, groupRepo.AddMembers(ctx, constants.DefaultAppID, "default", "customer-group", []string{customer.UserID}))
	...
	// 广播到customer组
	groupCtx := routing.NewRoute().WithAppID(constants.DefaultAppID).WithNamespace("default").WithGroupIDs([]string{"customer-group"}).Inject(ctx)
