...
func TestLegacySystem_OfflineStoreDrainKeyConsistency(t *testing.T) {
	...
	require.Equal(t, constants.DefaultAppID, routing.AppIDFromContext(legacyCtx), "EnsureRouteDefaults 应兜底 DefaultAppID")
	require.Equal(t, constants.DefaultNamespace, routing.NamespaceFromContext(legacyCtx), "EnsureRouteDefaults 应兜底 DefaultNamespace")
	...
	// FirstGroupIDFromContext 返回 ""，constants.NormalizeGroupID("") 补 DefaultGroupID，与 store 时一致
...
// constants.NormalizeGroupID("") 补 DefaultGroupID，保证 store/drain 维度一致
func TestLegacySystem_EmptyGroupID_P2PQueueNotBogus(t *testing.T) {
	...
	assert.Equal(t, "", firstGID, "WithGroup(\"\") 的 FirstGroupID 应为空串，经 constants.NormalizeGroupID 补 DefaultGroupID")
	// 关键：与 store 路径（groupIDs=nil → FirstGroupID=""）一致，两者 constants.NormalizeGroupID 后都补 DefaultGroupID
