 ...
 * @LastEditTime: 2025-12-02 00:00:00
 * @FilePath: \go-wsc\message_queue_repository.go
...
type MessageQueueRepository interface {
	// Enqueue 入队消息
	Enqueue(ctx context.Context, queueName string, msg *HubMessage) error

	// Dequeue 出队消息(阻塞式,带看门狗锁)
	Dequeue(ctx context.Context, queueName string, timeout time.Duration) (*HubMessage, error)

	// DequeueWithWatchdog 出队消息并启动看门狗锁
	DequeueWithWatchdog(ctx context.Context, queueName string, timeout time.Duration, processFunc func(*HubMessage) error) error
	...
	// Peek 查看队列头部消息(不移除)
	Peek(ctx context.Context, queueName string) (*HubMessage, error)
}
...
// Enqueue 入队消息
func (r *RedisMessageQueueRepository) Enqueue(ctx context.Context, queueName string, msg *HubMessage) error {
	if msg == nil {
...
// Dequeue 出队消息(阻塞式)
func (r *RedisMessageQueueRepository) Dequeue(ctx context.Context, queueName string, timeout time.Duration) (*HubMessage, error) {
	key := r.prefix + queueName
...
// DequeueWithWatchdog 出队消息并启动看门狗锁
// 使用处理队列和看门狗机制防止消息丢失
func (r *RedisMessageQueueRepository) DequeueWithWatchdog(ctx context.Context, queueName string, timeout time.Duration, processFunc func(*HubMessage) error) error {
	sourceKey := r.prefix + queueName
	processingKey := r.prefix + queueName + ":processing"
	lockKey := r.prefix + queueName + ":lock"
...
// Peek 查看队列头部消息(不移除)
func (r *RedisMessageQueueRepository) Peek(ctx context.Context, queueName string) (*HubMessage, error) {
	key := r.prefix + queueName
