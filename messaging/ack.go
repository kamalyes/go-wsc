 ...
 * @FilePath: \go-wsc\messaging\ack.go
...
type AckManager struct {
	...
	offlineRepo    spi.OfflineStore // 离线消息处理器
}
...
func NewAckManagerWithOptions(ackTimeout time.Duration, maxRetry int, expireDuration time.Duration, offlineRepo spi.OfflineStore) *AckManager {
	// 设置合理的默认ACK超时时间
...
// GetPendingCount 获取待确认消息数量（原子读取，零锁开销）
...
// 用于统一使用 Hub 的离线消息处理器
func (am *AckManager) SetOfflineRepo(handler spi.OfflineStore) {
	am.cfgMu.Lock()
