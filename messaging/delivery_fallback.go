 ...
 * @LastEditTime: 2026-09-22 10:42:00
 * @FilePath: \go-wsc\messaging\delivery_fallback.go
 * @Description: 投递兜底路由 —— 拒绝不等于丢弃（P0 分级送达基座）
 *
...
func (m *Manager) routeDeliveryFallback(msg *models.HubMessage, client *models.Client, targetUserID string) overload.FallbackAction {
	...
	if guarantee == models.GuaranteeEphemeral {
		...
		return overload.FallbackEphemeralDrop
	}
	...
	if userID == "" {
		...
		m.host.GetLogger().WarnContextKV(msg.ContextFrom(m.host.Context()), "投递兜底失败：消息无接收者信息",
			"message_id", msg.MessageID,
			"guarantee", guarantee,
		)
		m.host.GetOverloadMetrics().RecordUnrecoverable(guarantee)
		return overload.FallbackNone
	}
	...
	if m.offlineHandler == nil {
		...
		m.host.GetOverloadMetrics().RecordUnrecoverable(guarantee)
		m.host.GetLogger().WarnContextKV(msg.ContextFrom(m.host.Context()), "投递兜底失败：未配置离线消息处理器",
			"message_id", msg.MessageID,
			"user_id", userID,
			"guarantee", guarantee,
		)
	...
	m.StoreOfflineOnDeliveryFailure(offlineMsg, overload.ErrDeliveryFallback)
	m.host.GetOverloadMetrics().RecordOfflineFallback(guarantee)
	return overload.FallbackOffline
}
