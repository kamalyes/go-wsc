...
func (m *Manager) timeoutStaleSendingRecords() {
	...
	records, err := m.host.GetMessageSink().QueryRecords(ctx, &models.MessageRecordFilter{
		Status: &sending,
		Limit:  nodeAckScanLimit,
	})
	if err != nil {
		m.host.GetLogger().WarnContextKV(m.host.Context(), "ACK超时扫描：查询 sending 记录失败", "error", err)
		return
	}

	cutoff := time.Now().Add(-nodeAckTimeout)

	// 内存过滤超时记录（models.MessageRecordFilter 暂不支持时间范围，sending 停留是异常场景，量小）
	...
	// 多节点并发去重：所有 Pod 的扫描器都面对同一张共享记录表，若直接
	...
	claimed, claimErr := m.host.GetMessageSink().ClaimStaleSending(ctx, staleKeys, models.MessageSendStatusAckTimeout, models.FailureReasonAckTimeout, errNodeAckTimeout.Error())
	if claimErr != nil {
		m.host.GetLogger().WarnContextKV(m.host.Context(), "ACK超时扫描：认领超时记录失败",
	...
	m.host.GetLogger().WarnContextKV(m.host.Context(), "跨节点消息ACK超时，已标记待重试",
		"count", len(claimed),
	...
	for _, record := range stale {
		...
		if mErr != nil || msg == nil {
			m.host.GetLogger().WarnContextKV(m.host.Context(), "ACK超时扫描：反序列化消息失败，无法转存离线",
				"message_id", record.MessageID, "error", mErr)
			continue
		}
		// trace 恢复：MessageData 序列化了完整 HubMessage（含信封 trace_id），
		// StoreOfflineOnDeliveryFailure 内部经 msg.ContextFrom 恢复，转存离线日志可追溯原始发送链路
