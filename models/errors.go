 ...
 * @FilePath: \go-wsc\models\errors.go
...
func init() {
	...
	// 注册用户和认证错误
	errorx.RegisterError(ErrTypeUserOffline, "user is offline: %s")
	...
	// 注册ACK相关错误
	errorx.RegisterError(ErrTypeAckTimeout, "ack timeout for message %s")
...
const (
	// ErrMsgClientIDFormat 客户端ID错误消息格式
	ErrMsgClientIDFormat = "client_id: %s"

	// ErrMsgDecompressFailed 解压消息失败错误格式
	ErrMsgDecompressFailed = "decompress message failed: %w"
)
