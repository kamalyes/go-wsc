...
// 支持命名空间(namespace)+群组(group_id)隔离，类似 k8s 设计
type OfflineMessageRecord struct {
	ID             uint              `gorm:"primaryKey;autoIncrement;comment:主键,唯一标识离线消息记录" json:"id"`
	MessageID      string            `gorm:"column:message_id;size:64;not null;uniqueIndex:idx_message_ns_receiver;comment:业务消息ID" json:"message_id"`
	Sender         string            `gorm:"index;size:255;comment:发送者ID" json:"sender"`
	Receiver       string            `gorm:"index;size:255;not null;uniqueIndex:idx_message_ns_receiver;comment:接收者ID" json:"receiver"`
	Namespace      string            `gorm:"column:namespace;size:64;not null;default:'default';index:idx_namespace_receiver;uniqueIndex:idx_message_ns_receiver;comment:命名空间ID" json:"namespace"`
	GroupID        string            `gorm:"column:group_id;size:64;default:'';index:idx_namespace_receiver;comment:群组ID(空表示点对点消息)" json:"group_id,omitempty"`
...
func (OfflineMessageRecord) TableComment() string {
	return "WebSocket离线消息记录表-存储用户离线时接收到的消息信息用于消息投递和管理,支持namespace+group_id隔离"
}
