...
func TestRedisOnlineStatusRepositorySetClientOnline(t *testing.T) {
	...
	// 设置在线
	err := repo.SetClientOnline(ctx, client)
	...
	// 获取在线客户端列表
	clients, err := repo.GetUserClients(ctx, client.UserID)
	assert.NoError(t, err)
	require.NotEmpty(t, clients, "客户端列表不应为空")
...
func TestRedisOnlineStatusRepositorySetClientOffline(t *testing.T) {
	...
	// 设置客户端离线
	err = repo.SetClientOffline(ctx, client.ID)
	assert.NoError(t, err)
...
func TestRedisOnlineStatusRepositoryUpdateClientHeartbeat(t *testing.T) {
	...
	// 更新心跳
	err = repo.UpdateClientHeartbeat(ctx, client.ID)
	assert.NoError(t, err)
...
func TestRedisOnlineStatusRepositoryGetAllOnlineUsers(t *testing.T) {
	...
	for i := range 5 {
		...
		err := repo.SetClientOnline(ctx, clients[i])
		require.NoError(t, err)
	}
...
func TestRedisOnlineStatusRepositoryBatchSetClientsOnline(t *testing.T) {
	...
	// 批量设置在线
	err := repo.BatchSetClientsOnline(ctx, clients)
	assert.NoError(t, err)
	...
	for _, client := range clients {
		isOnline, err := repo.IsUserOnline(ctx, client.UserID)
		assert.NoError(t, err)
		assert.True(t, isOnline, "用户 %s 应该在线", client.UserID)
	}
...
func TestRedisOnlineStatusRepositoryBatchSetClientsOffline(t *testing.T) {
	...
	// 批量设置离线
	err := repo.BatchSetClientsOffline(ctx, clientIDs)
	assert.NoError(t, err)
	...
	for _, client := range clients {
		isOnline, err := repo.IsUserOnline(ctx, client.UserID)
		assert.NoError(t, err)
		assert.False(t, isOnline, "用户 %s 应该离线", client.UserID)
	}
}
...
func TestRedisOnlineStatusRepositoryGetOnlineUsersByType(t *testing.T) {
	...
	for i := range 3 {
		...
		err := repo.SetClientOnline(ctx, customerClients[i])
		require.NoError(t, err)
	}
	...
	for i := range 2 {
		...
		err := repo.SetClientOnline(ctx, agentClients[i])
		require.NoError(t, err)
	}
...
func TestRedisOnlineStatusRepositorySetAndGetUserNode(t *testing.T) {
	...
	// 获取用户节点
	retrievedNodeID, err := repo.GetUserNodes(ctx, client.UserID)
	assert.NoError(t, err)
	assert.Equal(t, client.NodeID, retrievedNodeID, "应返回正确的节点ID")

	// 测试不存在的用户
	_, err = repo.GetUserNodes(ctx, "non-existent-user")
...
func TestRedisOnlineStatusRepositoryCleanupExpired(t *testing.T) {
	...
	count, err := repo.CleanupExpired(ctx, client.NodeID)
	assert.NoError(t, err)
...
func TestRedisOnlineStatusRepositoryGetOnlineInfoNotFound(t *testing.T) {
	...
	// 获取不存在的用户信息
	info, err := repo.GetUserClients(ctx, "non-existent-user")
	assert.Error(t, err, "不存在的用户应返回错误")
