 ...
 * @FilePath: \go-wsc\overload\admission.go
...
// EvalInterval 返回评估周期（供上层对齐扫描节拍）
func (g *AdmissionGate) EvalInterval() time.Duration { return g.evalInterval }

// HighWatermark 返回高水位阈值（积压超过即升载）
func (g *AdmissionGate) HighWatermark() int64 { return g.highWatermark }

// LowWatermark 返回低水位阈值（积压低于即降载）
func (g *AdmissionGate) LowWatermark() int64 { return g.lowWatermark }

// CooldownLevels 返回降载冷却层级数（AIMD 迟滞参数）
func (g *AdmissionGate) CooldownLevels() int { return g.cooldownLevels }
