 ...
 * @FilePath: \go-wsc\overload\batch_sender.go
...
type BatchSender struct {
	hub UserMessageSender
	// failureCallback 批量发送失败回调（构造时由 Hub 注入，可为 nil）
	failureCallback BatchSendFailureCallback
	ctx             context.Context
	messages        map[string][]*models.HubMessage // userID -> messages
	mu              sync.RWMutex
}
...
// New 创建批量发送器
//
// failureCallback 由调用方（Hub）在构造时注入 —— 它来自 Hub 的未导出字段，
// 放在此处注入可避免为它新增一个公开访问器。
func New(user UserMessageSender, failureCallback BatchSendFailureCallback, ctx context.Context) *BatchSender {
	if ctx == nil {
	...
	return &BatchSender{
		...
		ctx:             ctx,
		messages:        make(map[string][]*models.HubMessage),
	}
}

// AddMessage 为用户添加一条消息（支持链式调用）
func (bs *BatchSender) AddMessage(userID string, msg *models.HubMessage) *BatchSender {
	syncx.WithLock(&bs.mu, func() {
		if bs.messages[userID] == nil {
			bs.messages[userID] = make([]*models.HubMessage, 0)
		}
...
func (bs *BatchSender) AddMessages(userID string, msgs ...*models.HubMessage) *BatchSender {
	syncx.WithLock(&bs.mu, func() {
		if bs.messages[userID] == nil {
			bs.messages[userID] = make([]*models.HubMessage, 0, len(msgs))
		}
...
func (bs *BatchSender) AddUserMessages(userMessages map[string][]*models.HubMessage) *BatchSender {
	syncx.WithLock(&bs.mu, func() {
...
func (bs *BatchSender) Clear() *BatchSender {
	syncx.WithLock(&bs.mu, func() {
		bs.messages = make(map[string][]*models.HubMessage)
	})
	return bs
}
...
func (bs *BatchSender) Execute() *BatchSendResult {
	bs.mu.RLock()
	// 复制一份数据，避免发送过程中被修改
	messagesCopy := make(map[string][]*models.HubMessage, len(bs.messages))
	for userID, msgs := range bs.messages {
		messagesCopy[userID] = make([]*models.HubMessage, len(msgs))
...
func (bs *BatchSender) sendUserMessages(userID string, msgs []*models.HubMessage, result *BatchSendResult) {
	...
	for _, msg := range msgs {
		...
		if sendResult.Success {
			...
			if sendResult.FinalError != nil {
				...
				// 调用批量发送失败回调
				if bs.failureCallback != nil {
					bs.failureCallback(userID, msg, sendResult.FinalError)
				}
			}
		}
	}
