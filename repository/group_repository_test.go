...
func TestCreateGroup(t *testing.T) {
	...
	t.Run("创建群组后可读取且字段一致", func(t *testing.T) {
		...
		got, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g-create")
		...
		// 创建后应出现在该命名空间的群组集合中
		namespaceGroups, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, "tenantA")
		require.NoError(t, err)
	...
	t.Run("Namespace 为空时归入 default 命名空间", func(t *testing.T) {
		...
		got, err := repo.GetGroup(ctx, models.DefaultAppID, models.DefaultNamespace, "g-default-tenant")
		require.NoError(t, err)
		...
		// 不应在其他命名空间下查到
		_, err = repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g-default-tenant")
		assert.ErrorIs(t, err, ErrGroupNotFound)

		// DefaultNamespace 命名空间群组集合应包含该群组
		namespaceGroups, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, models.DefaultNamespace)
		require.NoError(t, err)
	...
	t.Run("CreatedAt 为零值时自动填充当前时间", func(t *testing.T) {
		...
		got, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g-ts")
		require.NoError(t, err)
...
// TestGetGroup_NotFound 验证获取不存在的群组返回 ErrGroupNotFound
func TestGetGroup_NotFound(t *testing.T) {
	repo, cleanup := setupTestRepo(t)
	defer cleanup()
	ctx := context.Background()

	_, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "not-exist")
	assert.ErrorIs(t, err, ErrGroupNotFound, "不存在的群组应返回 ErrGroupNotFound")
}
...
func TestDisbandGroup(t *testing.T) {
	...
	require.NoError(t, repo.CreateGroup(ctx, g))
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-disband", []string{"u1", "u2", "u3"}))

	// 解散前确认成员与反向索引存在
	members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "g-disband")
	require.NoError(t, err)
	assert.Len(t, members, 3)
	for _, u := range []string{"u1", "u2", "u3"} {
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g-disband", u)
		require.NoError(t, err)
		assert.True(t, ok)
	}

	// 执行解散
	require.NoError(t, repo.DisbandGroup(ctx, models.DefaultAppID, "tenantA", "g-disband"))

	// 元信息应被删除
	_, err = repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g-disband")
	assert.ErrorIs(t, err, ErrGroupNotFound, "解散后元信息应被删除")
	...
	// 成员数量应为 0
	cnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-disband")
	require.NoError(t, err)
	assert.Equal(t, int64(0), cnt)

	// 各成员反向索引应被清理
	for _, u := range []string{"u1", "u2", "u3"} {
		groups, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantA", u)
		require.NoError(t, err)
...
func TestAddMembers(t *testing.T) {
	...
	t.Run("添加成员后成员集合与反向索引同步更新", func(t *testing.T) {
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-add", []string{"u1", "u2"}))

		// 成员集合应包含新增成员
		members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "g-add")
		require.NoError(t, err)
	...
	t.Run("重复添加成员应为幂等操作", func(t *testing.T) {
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-add", []string{"u1"}))
		cnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-add")
		require.NoError(t, err)
		assert.Equal(t, int64(2), cnt, "重复添加不应增加成员数")
	})

	t.Run("空成员列表为空操作且不报错", func(t *testing.T) {
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-add", []string{}))
	})
}
...
func TestRemoveMembers(t *testing.T) {
	...
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-rm", []string{"u1", "u2", "u3"}))

	// 移除部分成员
	require.NoError(t, repo.RemoveMembers(ctx, models.DefaultAppID, "tenantA", "g-rm", []string{"u1", "u2"}))

	// 剩余成员应为 u3
	members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "g-rm")
	require.NoError(t, err)
	assert.ElementsMatch(t, []string{"u3"}, members)

	cnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-rm")
	require.NoError(t, err)
	assert.Equal(t, int64(1), cnt)

	// 被移除成员不再属于群组
	for _, u := range []string{"u1", "u2"} {
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g-rm", u)
		require.NoError(t, err)
	...
	// 保留成员仍属于群组且反向索引仍在
	ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g-rm", "u3")
	require.NoError(t, err)
	assert.True(t, ok)
	groups, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantA", "u3")
	require.NoError(t, err)
	assert.Contains(t, groups, "g-rm")

	// 移除空列表不应报错
	require.NoError(t, repo.RemoveMembers(ctx, models.DefaultAppID, "tenantA", "g-rm", []string{}))
}
...
func TestGetMembers(t *testing.T) {
	...
	t.Run("无成员时返回空切片", func(t *testing.T) {
		members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "g-members")
		require.NoError(t, err)
	...
	t.Run("添加多个成员后全部返回", func(t *testing.T) {
		...
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-members", added))
	...
	t.Run("不存在的群组返回空切片而非报错", func(t *testing.T) {
		members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "not-exist")
		require.NoError(t, err)
		assert.Empty(t, members)
	})
}
...
func TestGetUserGroups(t *testing.T) {
	...
	for _, gid := range []string{"g1", "g2", "g3"} {
		...
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", gid, []string{"userX"}))
	}

	groups, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantA", "userX")
	require.NoError(t, err)
	assert.ElementsMatch(t, []string{"g1", "g2", "g3"}, groups, "应返回用户加入的全部群组")

	// 未加入任何群组的用户应返回空
	empty, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantA", "nobody")
	require.NoError(t, err)
...
func TestIsMember(t *testing.T) {
	...
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-is", []string{"u-in"}))

	t.Run("成员返回 true", func(t *testing.T) {
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g-is", "u-in")
		require.NoError(t, err)
		assert.True(t, ok)
	})

	t.Run("非成员返回 false", func(t *testing.T) {
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g-is", "u-out")
		require.NoError(t, err)
		assert.False(t, ok)
	})

	t.Run("不存在的群组对任意用户返回 false", func(t *testing.T) {
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantA", "not-exist", "u-in")
		require.NoError(t, err)
		assert.False(t, ok)
	})
}
...
func TestGetMemberCount(t *testing.T) {
	...
	// 初始成员数为 0
	cnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-count")
	require.NoError(t, err)
	assert.Equal(t, int64(0), cnt)

	// 添加 3 个成员
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g-count", []string{"u1", "u2", "u3"}))
	cnt, err = repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-count")
	require.NoError(t, err)
	assert.Equal(t, int64(3), cnt)

	// 移除 1 个后应为 2
	require.NoError(t, repo.RemoveMembers(ctx, models.DefaultAppID, "tenantA", "g-count", []string{"u1"}))
	cnt, err = repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g-count")
	require.NoError(t, err)
	assert.Equal(t, int64(2), cnt)

	// 不存在的群组成员数为 0
	cnt, err = repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "not-exist")
	require.NoError(t, err)
	assert.Equal(t, int64(0), cnt)
}
...
func TestGetNamespaceGroups(t *testing.T) {
	...
	t.Run("命名空间下创建多个群组后全部返回", func(t *testing.T) {
		...
		groups, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, "tenantA")
		require.NoError(t, err)
	...
	t.Run("不同命名空间的群组互不干扰", func(t *testing.T) {
		...
		aGroups, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, "tenantA")
		require.NoError(t, err)
		assert.NotContains(t, aGroups, "t-gB", "tenantB 的群组不应出现在 tenantA")

		bGroups, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, "tenantB")
		require.NoError(t, err)
		assert.Contains(t, bGroups, "t-gB")
	})

	t.Run("空命名空间返回空切片", func(t *testing.T) {
		empty, err := repo.GetNamespaceGroups(ctx, models.DefaultAppID, "empty-tenant")
		require.NoError(t, err)
		assert.Empty(t, empty)
	})
}
...
func TestNamespaceIsolation(t *testing.T) {
	...
	t.Run("元信息按命名空间隔离", func(t *testing.T) {
		gA, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g1")
		require.NoError(t, err)
		assert.Equal(t, "A群", gA.Name)
		assert.Equal(t, "ownerA", gA.OwnerID)

		gB, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantB", "g1")
		require.NoError(t, err)
	...
	t.Run("成员不跨命名空间泄漏", func(t *testing.T) {
		...
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g1", []string{"userA"}))
		// tenantB 的 g1 加入 userB
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantB", "g1", []string{"userB"}))

		// tenantA.g1 成员只应包含 userA
		aMembers, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", "g1")
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"userA"}, aMembers)

		// tenantB.g1 成员只应包含 userB
		bMembers, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantB", "g1")
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"userB"}, bMembers)

		// userA 不应是 tenantB.g1 的成员
		ok, err := repo.IsMember(ctx, models.DefaultAppID, "tenantB", "g1", "userA")
		require.NoError(t, err)
		assert.False(t, ok, "userA 不应跨命名空间出现在 tenantB.g1")

		// userB 不应是 tenantA.g1 的成员
		ok, err = repo.IsMember(ctx, models.DefaultAppID, "tenantA", "g1", "userB")
		require.NoError(t, err)
	...
	t.Run("反向索引按命名空间隔离", func(t *testing.T) {
		...
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantB", "g1", []string{"userA"}))

		aGroups, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantA", "userA")
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"g1"}, aGroups, "tenantA 下 userA 只应有一个 g1")

		bGroups, err := repo.GetUserGroups(ctx, models.DefaultAppID, "tenantB", "userA")
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"g1"}, bGroups, "tenantB 下 userA 也只应有一个 g1")

		// 两个命名空间的成员数量各自独立
		aCnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantA", "g1")
		require.NoError(t, err)
		assert.Equal(t, int64(1), aCnt, "tenantA.g1 成员数应为 1")

		bCnt, err := repo.GetMemberCount(ctx, models.DefaultAppID, "tenantB", "g1")
		require.NoError(t, err)
		assert.Equal(t, int64(2), bCnt, "tenantB.g1 成员数应为 2（userA + userB）")
	})

	t.Run("解散某命名空间群组不影响另一命名空间同名群组", func(t *testing.T) {
		require.NoError(t, repo.DisbandGroup(ctx, models.DefaultAppID, "tenantA", "g1"))

		// tenantA.g1 应已不存在
		_, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g1")
		assert.ErrorIs(t, err, ErrGroupNotFound)
...
// 重复创建应返回 ErrGroupExisted；不同命名空间同名 groupID 不冲突
func TestCreateGroupDuplicate(t *testing.T) {
	...
	t.Run("同命名空间重复创建返回 ErrGroupExisted", func(t *testing.T) {
		err := repo.CreateGroup(ctx, &Group{GroupID: "g-dup", Namespace: "tenantA", Name: "覆盖尝试"})
		assert.ErrorIs(t, err, ErrGroupExisted, "同命名空间重复创建应返回 ErrGroupExisted")

		// 原群组元信息不应被覆盖
		got, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "g-dup")
		require.NoError(t, err)
	...
	t.Run("不同命名空间同名 groupID 不冲突", func(t *testing.T) {
		...
		got, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantB", "g-dup")
		require.NoError(t, err)
...
func TestGetAllNamespaces(t *testing.T) {
	...
	t.Run("无群组时返回空", func(t *testing.T) {
		namespaces, err := repo.GetAllNamespaces(ctx, models.DefaultAppID)
		require.NoError(t, err)
...
func TestGetMultiGroupMembers(t *testing.T) {
	...
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g1", []string{"u1", "u2"}))
	require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", "g2", []string{"u2", "u3"}))
	// g3 无成员

	t.Run("批量返回所有群组成员", func(t *testing.T) {
		result, err := repo.GetMultiGroupMembers(ctx, models.DefaultAppID, "tenantA", []string{"g1", "g2", "g3"})
		require.NoError(t, err)
	...
	t.Run("空 groupIDs 返回 nil", func(t *testing.T) {
		result, err := repo.GetMultiGroupMembers(ctx, models.DefaultAppID, "tenantA", []string{})
		require.NoError(t, err)
		assert.Nil(t, result)
	})

	t.Run("包含不存在的群组时该 key 返回空切片", func(t *testing.T) {
		result, err := repo.GetMultiGroupMembers(ctx, models.DefaultAppID, "tenantA", []string{"g1", "not-exist"})
		require.NoError(t, err)
	...
	t.Run("跨命名空间隔离：查 tenantB 的群组应返回空", func(t *testing.T) {
		result, err := repo.GetMultiGroupMembers(ctx, models.DefaultAppID, "tenantB", []string{"g1", "g2"})
		require.NoError(t, err)
...
func TestCreateGroupReserved(t *testing.T) {
	...
	t.Run("业务组用 __ 前缀返回 ErrGroupReserved", func(t *testing.T) {
		err := repo.CreateGroup(ctx, &Group{GroupID: "__mygroup__", Namespace: "tenantA", OwnerID: "o"})
		assert.ErrorIs(t, err, ErrGroupReserved, "__ 前缀为系统保留名，业务组应拒绝")

		// 确认未创建
		_, err = repo.GetGroup(ctx, models.DefaultAppID, "tenantA", "__mygroup__")
...
func TestEnsureSystemGroup(t *testing.T) {
	...
	t.Run("非系统组名返回 ErrGroupReserved", func(t *testing.T) {
		err := repo.EnsureSystemGroup(ctx, models.DefaultAppID, "tenantA", "normal-group")
		assert.ErrorIs(t, err, ErrGroupReserved, "仅允许 __ 前缀系统组名")
	})

	t.Run("首次创建系统组成功", func(t *testing.T) {
		err := repo.EnsureSystemGroup(ctx, models.DefaultAppID, "tenantA", SystemGroupAgents)
		require.NoError(t, err)

		// 确认已创建且可查询
		g, err := repo.GetGroup(ctx, models.DefaultAppID, "tenantA", SystemGroupAgents)
		require.NoError(t, err)
	...
	t.Run("系统组可正常添加和查询成员", func(t *testing.T) {
		require.NoError(t, repo.EnsureSystemGroup(ctx, models.DefaultAppID, "tenantA", SystemGroupObservers))
		require.NoError(t, repo.AddMembers(ctx, models.DefaultAppID, "tenantA", SystemGroupObservers, []string{"u1", "u2"}))

		members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantA", SystemGroupObservers)
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"u1", "u2"}, members)
	})
	...
	t.Run("全局观察者系统组 tenant 为空", func(t *testing.T) {
		// 全局观察者系统组 tenant=""
		err := repo.EnsureSystemGroup(ctx, models.DefaultAppID, "", SystemGroupObservers)
		require.NoError(t, err)

		g, err := repo.GetGroup(ctx, models.DefaultAppID, "", SystemGroupObservers)
		require.NoError(t, err)
		assert.Equal(t, "system", g.OwnerID)
	})
	...
	t.Run("并发 ensure 同一系统组幂等", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			go func(idx int) {
				defer wg.Done()
				errs[idx] = repo.EnsureSystemGroup(ctx, models.DefaultAppID, "tenantC", SystemGroupAgents)
			}(i)
		}
		wg.Wait()
		...
		// 确认只创建了一个
		members, err := repo.GetMembers(ctx, models.DefaultAppID, "tenantC", SystemGroupAgents)
		require.NoError(t, err)
...
func TestGetGroupNamespace(t *testing.T) {
	...
	t.Run("反查存在的群组", func(t *testing.T) {
		namespace, err := repo.GetGroupNamespace(ctx, models.DefaultAppID, "group-1")
		require.NoError(t, err)
		assert.Equal(t, "tenantA", namespace)
	})

	t.Run("反查不存在的群组返回 ErrGroupNotFound", func(t *testing.T) {
		_, err := repo.GetGroupNamespace(ctx, models.DefaultAppID, "nonexistent")
		assert.ErrorIs(t, err, ErrGroupNotFound)
	})
...
func TestGetMultiGroupNamespaces(t *testing.T) {
	...
	t.Run("批量反查全部存在", func(t *testing.T) {
		result, err := repo.GetMultiGroupNamespaces(ctx, models.DefaultAppID, []string{"g1", "g2", "g3"})
		require.NoError(t, err)
	...
	t.Run("批量反查部分不存在", func(t *testing.T) {
		result, err := repo.GetMultiGroupNamespaces(ctx, models.DefaultAppID, []string{"g1", "nonexistent"})
		require.NoError(t, err)
		assert.Equal(t, "tenantA", result["g1"])
	...
	t.Run("空输入返回 nil", func(t *testing.T) {
		result, err := repo.GetMultiGroupNamespaces(ctx, models.DefaultAppID, nil)
		require.NoError(t, err)
		assert.Nil(t, result)
	})
}
...
func TestDisbandGroupDeletesReverseMapping(t *testing.T) {
	...
	// 确认反查可用
	namespace, err := repo.GetGroupNamespace(ctx, models.DefaultAppID, "to-delete")
	require.NoError(t, err)
	assert.Equal(t, "tenantA", namespace)

	// 删除群组
	require.NoError(t, repo.DisbandGroup(ctx, models.DefaultAppID, "tenantA", "to-delete"))

	// 反查应返回 ErrGroupNotFound（反向映射已清理）
	_, err = repo.GetGroupNamespace(ctx, models.DefaultAppID, "to-delete")
	assert.ErrorIs(t, err, ErrGroupNotFound, "删除群组后反向映射应同步清理")
}
