...
for i = 1, clientCount do
    ...
    if #parts == 4 then
        ...
        -- 检查用户是否还有其他客户端
        local remainingCount = redis.call('ZCARD', userClientsKey)
...
func (r *RedisOnlineStatusRepository) IsUserOnline(ctx context.Context, userID string) (bool, error) {
	count, err := r.client.ZCard(ctx, r.GetUserClientsKey(userID)).Result()
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (r *RedisOnlineStatusRepository) GetAllOnlineUsers(ctx context.Context) ([]string, error) {
	// 使用 ZRANGEBYSCORE 只获取未过期的用户（score > 当前时间）
	currentTime := time.Now().Unix()
	return r.client.ZRangeByScore(ctx, r.GetAllUsersSetKey(), &redis.ZRangeBy{
		Min: fmt.Sprintf("%d", currentTime),
		Max: "+inf",
	}).Result()
}
...
func (r *RedisOnlineStatusRepository) GetOnlineUsersByType(ctx context.Context, userType models.UserType) ([]string, error) {
	// 使用 ZRANGEBYSCORE 只获取未过期的用户（score > 当前时间）
	currentTime := time.Now().Unix()
	return r.client.ZRangeByScore(ctx, r.GetUserTypeSetKey(userType), &redis.ZRangeBy{
		Min: fmt.Sprintf("%d", currentTime),
		Max: "+inf",
	}).Result()
}
